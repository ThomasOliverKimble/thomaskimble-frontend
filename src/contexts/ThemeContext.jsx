...
export const ThemeProvider = ({ children }) => {
  ...
  const [primaryColor, setPrimaryColor] = useState(
    localStorage.getItem("themeColor") || defaultTheme.color
  );
  const [primaryHighlightColor, setPrimaryHighlightColor] = useState(
    localStorage.getItem("themeHighlightColor") || defaultTheme.color + "90"
  ); // Assuming the highlight is a 90% opacity version
  const [mediaPath, setMediaPath] = useState(
    localStorage.getItem("themeMediaPath") || defaultTheme.mediaPath
  );
